-- Ordena o fitment exato acima do deduzido.
--
-- `origem` ganhou um terceiro valor em 0010: 'auto_exato' é o vínculo que a
-- descrição do ERP afirma pela cilindrada em número, e o agente pode afirmar
-- compatibilidade nele. A ordenação anterior tratava 'auto' e 'auto_exato'
-- como a mesma coisa (ambos "is not null"), então, com a moto do cliente
-- conhecida, uma peça que o agente teria de hedgear podia ficar acima de uma
-- que ele poderia afirmar.
--
-- O resto da função e identico a 0003 — a calibração (cobertura de tokens,
-- strict_word_similarity, bônus de tipo, limiares presos com SET) nao mudou.
-- Fitment continua ordenando e NUNCA filtrando.
...
as $$
  ...
  select p.id,
         ...
         p.unidade,
         p.estoque,
         -- 'humano' = balcão confirmou que serve; 'auto_exato' = a descrição
         -- do ERP diz a cilindrada e ela bate; 'auto' = deduzido do modelo sem
         -- número, vale só como pista de ordenação; 'nenhum' = não mapeado.
  ...
  order by (pm.origem = 'humano') desc nulls last,
           (pm.origem = 'auto_exato') desc nulls last,
